...
function SideBar() {
    ...
    const deleteAccount = async () => {
        const res = await fetch(`http://localhost:9000/api/user/delete/` + currentUser.id, {
            method: "DELETE"
        });
    ...
    useEffect(() => {
        const getFriends = async () => {
          const res = await fetch(`http://localhost:9000/api/friend`, {
            headers: {
    ...
    return (
        <div className='flex items-start justify-center'>
            <div ref={side} className='bg-[#000000cc] p-5 w-fit h-[500px] rounded-bl-lg rounded-tl-lg mobile:invisible mobile:absolute z-40 mobile:right-[50%]'>
